...
The initial HTTP client supports `http://` base URLs without a path, GET, POST, PUT, DELETE, request headers, response headers, query path construction, and whole-body responses. The initial TCP client is synchronous and intended as a low-level building block. TLS, redirects, HTTP streaming, async TCP, UDP, and WebSocket support are planned follow-up work.
