...
TEST_CASE("HttpClient async_get returns response via callback") {
    LocalHttpServer server;

    auto client_r = nexus::net::HttpClient::create(server.base_url());
    REQUIRE(client_r.ok());
    ...
    client.async_get("/health", [&](nexus::Result<nexus::net::HttpResponse> r) {
        ok = r.ok();
        if (r.ok()) {
            status_code = r.value().status_code;
            body = std::move(r).value().body;
        }
        done.emit();
    });

    CHECK(done.wait(5000));
    REQUIRE(ok);
    CHECK(status_code == 200);
    CHECK(body == "ok");
}

TEST_CASE("HttpClient async_post sends body and content type") {
    LocalHttpServer server;
    ...
    client.async_post("/echo", "hello", "text/plain",
        [&](nexus::Result<nexus::net::HttpResponse> r) {
            ok = r.ok();
            if (r.ok()) {
                status_code = r.value().status_code;
                body = std::move(r).value().body;
...
TEST_CASE("HttpClient async_get rejects invalid base URL") {
    auto client = nexus::net::HttpClient::create("not-a-url");
    REQUIRE_FALSE(client.ok());
    CHECK(client.status().code() == nexus::StatusCode::kInvalidArgument);
}

TEST_CASE("HttpClient async_close fires handler") {
    LocalHttpServer server;
    ...
    bool ok = false;
    client.async_close([&](nexus::Status s) {
        ok = s.ok();
        closed.emit();
    });
...
TEST_CASE("HttpClient destroy with pending request does not hang") {
    LocalHttpServer server;
    ...
    auto destroy_start = std::chrono::steady_clock::now();
    {
        ...
        // Start a request to a slow endpoint (will be cancelled by destroy)
        client.async_get("/health", [](nexus::Result<nexus::net::HttpResponse>) {});

        destroy_start = std::chrono::steady_clock::now();
        // client goes out of scope — destructor must not hang
    }
    auto elapsed = std::chrono::steady_clock::now() - destroy_start;
    CHECK(elapsed < std::chrono::milliseconds(2000));
}

TEST_CASE("HttpClient close from callback does not deadlock") {
    LocalHttpServer server;
