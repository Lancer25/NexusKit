...
TEST_CASE("HID device rejects empty paths") {
    const auto device = nexus::hid::HidDevice::open_path("");

    REQUIRE_FALSE(device.ok());
    CHECK(device.status().code() == nexus::StatusCode::kInvalidArgument);
}

TEST_CASE("HID device reports closed operations") {
    nexus::hid::HidDevice device;
...
TEST_CASE("HID device validates report sizes") {
    nexus::hid::HidDevice device;
